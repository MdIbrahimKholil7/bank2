...
function updateBalance(input,isAdd){
    ...
    if(isAdd==false){
        ...
        const subTotal=getInnerText('deposite-blance') - getInput(input);
        console.log(subTotal)
       
        const total=getInnerText('total-blance')+subTotal;
        console.log(total)
        return document.getElementById('total-blance').innerText=total
    }else{
        const fullTotal=(getInnerText('total-blance') + input);
        return document.getElementById('total-blance').innerText=fullTotal
    }
}
// adding deposite  
